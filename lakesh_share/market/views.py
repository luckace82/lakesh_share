...
def screener_view(request):
    ...
    if above_ma20:
        queryset = queryset.filter(ma_20__isnull=False)
    if below_ma20:
        queryset = queryset.filter(ma_20__isnull=False)
...
def download_portfolio_report(request):
    ...
    <body>
        ...
        <div class="footer">
            <p>Generated by DhanLabh Portfolio Manager</p>
        </div>
    </body>
    </html>
    """
...
def generate_nepse_insights(request):
    ...
    from .scraper_tasks import generate_nepse_insights as generate_insights_task
    from django.utils import timezone
    ...
    try:
        ...
        generate_insights_task.apply_async(args=[is_market_closed])
